...
export default class JSONMapperClass {
  ...
  conversion(source: any = {}, extAttrs = {}, mapperRules: any) {
    ...
    targetKeys.reduce((origin: any, targetKey) => {
      ...
      const isObject = lodash.isObject(sourceKey)

      if (isDirect) {
        origin[targetKey] = lodash.result(source, sourceKey)
      } else if (isArray) {
        const targetKeyArr: any = sourceKey
        origin[targetKey] = targetKeyArr
          .map((item: string) => source[item])
      ...
      if (
        origin[targetKey] === null ||
        origin[targetKey] === undefined ||
        origin[targetKey] === ''
      ) {
        delete origin[targetKey]
      }

      lodash.extend(origin, source.extendsExports || {})

      return origin
    }, target)

    lodash.assign(target, extAttrs)
  ...
  static mixinTask(options: any, source: any = {}) {
    ...
    switch (options.type) {
      ...
      case 'moment-random-unique-id':
        return `${moment().format('X')}-${Math.random()
          .toString(32)
          .substr(2, 6)}`
      case 'array-select':
  ...
  static operationMatch(lValue: any, operation: string, rValue: any) {
    ...
    switch (operation) {
      ...
      case 'elt':
        ...
        break
      case 'eq':
        result = lValue == rValue // @ts-ignore
        break
      case 'neq':
        // @ts-ignore
        result = lValue != rValue
  ...
  static switchMapTask(options: any, source: any): any {
    ...
    if (selectOptions && selectOptions.length) {
      const matchItem = lodash.find(selectOptions, itemOption => {
        const leftValue = lodash.result(source, itemOption.leftSelect)
        const rightValue = itemOption.rightValue
          ? itemOption.rightValue
          : itemOption.rightSelect
          ? lodash.result(source, itemOption.rightSelect)
          : null
  ...
  static ifMapTask(option: any, source: any) {
    ...
    const targetResult = lodash.result(source, targetSelect)
    const defaultResult = lodash.result(source, defaultSelect) || defaultValue

    return !lodash.isEmpty(targetResult) ? targetResult : defaultResult
  }
  ...
  static randomProportionMapTask(option: any, source: any) {
    ...
    if (options && options.length) {
      const matchItems = lodash.filter(options, itemOption => {
        const matchSelects = itemOption.matchSelects
        const isMatch = lodash.every(matchSelects, item => lodash.result(source, item))
        if (isMatch) {
      ...
      const matchItem = lodash.find(matchItems, item => {
        randomValue = randomValue - item.rate
        return randomValue <= 0
      })

      result = !lodash.isEmpty(matchItem) ? matchItem.result : defaultValue
    }
  ...
  static momentFormatTask(option: any, source: any) {
    ...
    const sourceValue = lodash.result(source, select)
  ...
  static injectArrTask(option: any, source: any) {
    ...
    if (sourceValue && lodash.isArray(sourceValue)) {
  ...
  static injectArrAndTemplateRenderTask(option: any, source: any) {
    ...
    let result: any = lodash.cloneDeep(afterInjectArr)
    ...
    for (const key in sourcePathObj) {
      if (sourcePathObj.hasOwnProperty(key)) {
        ...
        const matchStr = `<!-${key}->`
        const value = lodash.result(source, selectPath)
    ...
    // 移除未替换的元素
    result = lodash.filter(result, item => !/<!-/.test(item))

    // 唯一值
    result = lodash.uniq(result)
  ...
  static arrayStrMapperTask(option: any, source: any) {
    ...
    const sourceValues: any = lodash.result(source, select)

    if (sourceValues) {
      const arr = lodash.map(sourceValues, key => mapper[key])
      const filterArr = arr.filter(item => item)

      return filterArr
    }

    return lodash.cloneDeep(option.default)
  }
  ...
  static arraySelectTask(option: any, source: any) {
    ...
    let targetValues = lodash.cloneDeep(option.default)
    targetValues = lodash.isArray(targetValues) ? targetValues : []

    const selectsLen = selects.length

    for (let index = 0; index < selectsLen; index++) {
      const selectPath = selects[index]
      const result = lodash.result(source, selectPath)
  ...
  static strMaxLenLimitTask(option: any, source: any) {
    ...
    const value = lodash.result(source, select)
  ...
  static str2numTask(option: any, source: any) {
    ...
    if (select) {
      value = lodash.toNumber(lodash.result(source, select)) || value
    }
  ...
  static splitStr2Aarr(option: any, source: any) {
    ...
    const selectValue: string = lodash.result(source, select)
    ...
    if (itemType === 'number') {
      valueArr = lodash.map(valueArr, item => lodash.toNumber(item))
    }
